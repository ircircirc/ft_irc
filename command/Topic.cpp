...
void ConfigManager::processTopic(std::vector<std::string> &commandAndParams, int clientFd)
{
	...
	{
		serverToClientMsg[clientFd] += ":irc.local 461 " + memberMap[fdNicknameMap[clientFd]].nickname + " TOPIC :Not enough parameters.\r\n";
        setWriteEvent(clientFd);
    	return;
	}
	...
	{
		serverToClientMsg[clientFd] += ":irc.local 403 " + memberMap[fdNicknameMap[clientFd]].nickname + " " + channel + " :No such channel\r\n";
		setWriteEvent(clientFd);
		return;
	}
	...
	if (commandAndParams.size() == 2)
	{
		serverToClientMsg[clientFd] += ":irc.local 332 " + memberMap[fdNicknameMap[clientFd]].nickname + " " + channel + "\r\n";
		setWriteEvent(clientFd);
		return;
	}
