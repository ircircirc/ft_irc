...
void ConfigManager::checkRegister(int clientFd)
{
    ...
    // std::cout << unregisterMember.nickname << " " << unregisterMember.username << std::endl;
    // std::cout << unregisterMember.nickname.size() << " " << unregisterMember.username.size() << std::endl;
    // std::cout << "PEDING STATE: " << unregisterMember.pendingCloseSocket << std::endl;
